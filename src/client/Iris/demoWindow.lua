return function(Iris)
	...
	local widgetDemos = {
		...
		Input = function()
			...
			Iris.InputColor4({ "InputColor4", UseFloats:get(), UseHSV:get() }, { color = sharedColor })
			Iris.SameLine()
	...
	local function widgetStateInteractivity()
		...
		Iris.Checkbox({ "Widget and Code Coupled State" }, { isChecked = checkboxState1 })
		...
		Iris.Checkbox({ "ComputedState (dynamic coupling)" }, { isChecked = checkboxState2 })
		Iris.Checkbox({ "Inverted of above checkbox" }, { isChecked = checkboxState3 })
