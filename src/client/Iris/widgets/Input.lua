return function(Iris, widgets)
		...
		UpdateState = function(thisWidget)
			...
			local newTextX = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.X or 1) >= 1 and "%d"
						or "%f"
					),
			...
			local newTextY = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Y or 1) >= 1 and "%d"
						or "%f"
					),
			...
			local newTextZ = string.format(
				thisWidget.arguments.Format
					or (
						(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Z or 1) >= 1 and "%d"
						or "%f"
					),
	...
	local abstractInputUDim = widgets.extend(abstractInputVector2, {
		...
		UpdateState = function(thisWidget)
			...
			local formatTextOffset = thisWidget.arguments.Format
				or (
					(thisWidget.arguments.Increment and thisWidget.arguments.Increment.Offset or 1) >= 1 and "%d"
					or "%f"
				)
	...
	do -- Iris.DragNum
		...
		widgets.PluginUserInput.InputEnded:Connect(function(inputObject)
		...
		local function updateActiveDrag()
			local currentMouseX = widgets.PluginUserInput.GetMouseLocation().X
		...
		widgets.PluginUserInput.InputChanged:Connect(updateActiveDrag)
	...
	do -- Iris.SliderNum
		...
		local function updateActiveSlider()
			...
			local PositionRatio = (
				widgets.PluginUserInput.GetMouseLocation().X - (InputFieldContainer.AbsolutePosition.X + GrabPadding)
			...
			Generate = function(thisWidget)
				...
				InputFieldG.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						local newValueColor
						if thisWidget.arguments.UseHSV then
							local H, _, V = thisWidget.state.color.value:ToHSV()
				...
				InputFieldB.FocusLost:Connect(function()
					...
					if newValue ~= nil then
						...
						local newValueColor
						if thisWidget.arguments.UseHSV then
							local H, S, _ = thisWidget.state.color.value:ToHSV()
