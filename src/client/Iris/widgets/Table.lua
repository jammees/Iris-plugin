	...
	Iris.WidgetConstructor("Table", {
		...
		Update = function(thisWidget)
			...
			if thisWidget.arguments.RowBg == false then
				...
				for rowColumnIndex, v in thisWidget.CellInstances do
					...
					v.BackgroundTransparency = if currentRow % 2 == 0
						then Iris._config.TableRowBgAltTransparency
						else Iris._config.TableRowBgTransparency
				end
			end
		...
		ChildAdded = function(thisWidget)
			...
			local selectedParent =
				thisWidget.ColumnInstances[((thisWidget.RowColumnIndex - 1) % thisWidget.InitialNumColumns) + 1]
			...
			if thisWidget.arguments.BordersInner == false then
				...
				widgets.UIStroke(
					cell,
					0.5,
					Iris._config.TableBorderLightColor,
					Iris._config.TableBorderLightTransparency
				)
			...
			if thisWidget.arguments.RowBg ~= false then
				...
				local color = if currentRow % 2 == 0
					then Iris._config.TableRowBgAltColor
					else Iris._config.TableRowBgColor
				local transparency = if currentRow % 2 == 0
					then Iris._config.TableRowBgAltTransparency
					else Iris._config.TableRowBgTransparency
