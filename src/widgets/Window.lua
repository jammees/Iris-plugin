...
return function(Iris, widgets: Types.WidgetUtility)
	local function relocateTooltips()
		...
		local mouseLocation = widgets.PluginUserInput.GetMouseLocation()
	...
	widgets.PluginUserInput.InputChanged:Connect(relocateTooltips)
	...
	widgets.PluginUserInput.InputBegan:Connect(function(input, gameProcessedEvent)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			if isInsideResize and not isInsideWindow and anyFocusedWindow then
				...
				local cursorPosition = widgets.PluginUserInput.GetMouseLocation() - midWindow
	...
	widgets.PluginUserInput.InputChanged:Connect(function(input)
		if isDragging then
			local mouseLocation
			if input.UserInputType == Enum.UserInputType.Touch then
				...
				mouseLocation = widgets.PluginUserInput.GetMouseLocation()
			end
		...
		if isResizing then
			...
			if input.UserInputType == Enum.UserInputType.Touch then
				...
				mouseDelta = widgets.PluginUserInput.GetMouseLocation() - lastCursorPosition
			end
		...
		lastCursorPosition = widgets.PluginUserInput.GetMouseLocation()
	end)

	widgets.PluginUserInput.InputEnded:Connect(function(input, _)
		if
	...
	Iris.WidgetConstructor("Window", {
		...
		Generate = function(thisWidget)
			...
			-- FIXME: Inputs do not get registered unless ZIndexBehaviour is set to Global
			...
			-- 		return
			-- 	end
			-- 	print("passed mousemove or keyboard check")
			-- 	if thisWidget.state.isUncollapsed.value then
			-- 		print("focused")
			-- 		Iris.SetFocusedWindow(thisWidget)
			-- 	end
			-- 	print(not thisWidget.arguments.NoMove)
			-- 	print(input.UserInputType == Enum.UserInputType.MouseButton1)
			-- 	print(not thisWidget.arguments.NoMove and input.UserInputType == Enum.UserInputType.MouseButton1)
			-- 	if not thisWidget.arguments.NoMove and input.UserInputType == Enum.UserInputType.MouseButton1 then
			-- 		print("draggin")
			-- 		dragWindow = thisWidget
			-- 		isDragging = true
			-- 		moveDeltaCursorPosition = widgets.PluginUserInput.GetMouseLocation() - thisWidget.state.position.value
			-- 	end
			-- end)
			...
			WindowButton.MouseButton1Down:Connect(function()
				...
				if not thisWidget.arguments.NoMove then
					dragWindow = thisWidget
					isDragging = true
					moveDeltaCursorPosition = widgets.PluginUserInput.GetMouseLocation()
